---
adr: kiro
status: current as of 2026-08-14 · credits captured and measured, now stored in `ledger/kiro/` (P2, v0.51) · NO token spine · upgrade-watch armed
...
> **⚠ Kiro KEEPS its transcript→`calls` leg — the one deviation from P5 (v0.51).**
> For claude and copilot that leg was a duplicate. **Kiro IDE has no metric twin:**
> `parse_kiro_ide_metrics` reads `devdata.sqlite`, which is absent on every install ever
> probed, so the leg is the ONLY reader of `tokens_generated.jsonl` — the only file kiro
> IDE writes. Retiring it would not de-duplicate kiro; it would **end kiro IDE capture**.
>
> Two consequences the retirement's justification did not name:
> **(1)** the routing decision above loses the rows it routes — with nothing captured
> there is nothing to route, and the mechanism goes unexercised;
> **(2)** the upgrade-watch loses its baseline, and noticing the day Kiro ships a real
> store is easiest against rows that are still arriving.
>
> The stated reason for removal — 28 rows, 1,576 in / **0 out**, model `"agent"`, one
> byte-identical 6-row block repeated — is that the rows are **unsummable**, and that is
> already handled: `ABSENT_SPINES["kiro"]` keeps them out of every total, so they cost
> nothing and render `—` with a reason rather than a `0`. **Unsummable is not worthless.**
>
> Kept as the smaller, reversible choice and flagged for ratification;
> `tests/test_calls_retired.py` pins it so implementing the spec as written is a
