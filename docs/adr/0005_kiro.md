---
adr: kiro
status: current as of 2026-08-15 · credits captured and measured, stored in `ledger/kiro/` (P2, v0.51) · the IDE store's rows moved there too (KIRO-CALLS-LEG, ratified) · NO token spine · upgrade-watch armed
...
flowchart TD
    ...
    JL --> MC["ledger/kiro/ ide-log row → MACHINE ledger (~/.cage)<br/>never a project .cage/"]
...
```text
  ...
  KIRO IDE  -- knows NOTHING (no project, no session, no timestamp) --------------
    dev_data/tokens_generated.jsonl
        +-- ledger/kiro/ ----------> ide-log rows, MACHINE ledger (~/.cage) ONLY
...
- **Two stores, opposite attribution properties.** Applying one rule to both destroys
  ...
  |---|---|---|---|
  | **IDE** `dev_data/tokens_generated.jsonl` | `parse_kiro_ide_log_metrics` | no project · `session="kiro"` · no `ts` | **machine fact** |
...
- **Routing (inherited, never re-decided).** IDE rows go to `~/.cage` — never a project
  ...
  **The routing rule survived the row kind changing under it** (KIRO-CALLS-LEG): IDE rows
  are `ide-log` metric rows now rather than `calls` rows, and the sink resolution is
  untouched, because `import_kiro` already runs against the routed sink AS its `root`.
  That is why this bullet says *inherited, never re-decided* — nothing about the routing
  ever depended on which kind the row was.
...
|---|---|---|
| `ide-log` | IDE `dev_data/tokens_generated.jsonl` (append-only JSONL) — **the only IDE store that exists**, and since KIRO-CALLS-LEG the only place its rows land | per-call `promptTokens`/`generatedTokens` verbatim, `surface="ide"`, `session="kiro"`, machine ledger only. Coarse by the vendor's doing: output usually 0, model usually `"agent"`. No `ts` in the store, so the row carries import time — exactly as the retired `calls` row did |
...
Routing is inherited, never re-decided: `ide`/`ide-log` rows ride the routed kiro sink
(machine ledger); `cli-conv`/`cli-turn` ride the same workspace scoping the `credits` leg
resolves.
