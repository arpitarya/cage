---
adr: cli
status: current as of 2026-08-15 · 14 top-level entries · 23 addressable commands · an example per command AND rendered output for every printing view · gated bidirectionally against the live parser · `matrix` re-ratified under ADR-MATRIX 2026-08-15, not yet returned to the live parser — the removed-verbs table and Known gaps note it, the §2 reference tables do not (nothing to gate yet)
...
**In one line:** five verbs do the daily work, six groups hold everything else, and the
front door shows you the five rather than all twenty-nine.
...
flowchart TD
    CLI["cage"]
    CLI --> D["THE DAILY FIVE<br/>shown at the front door"]
    CLI --> G["FIVE GROUPS<br/>named, not expanded"]
    ...
    D --> D5["clean — prune stale state/ (STATE-RETENTION)"]
    G --> G1["insights — 5 derived views"]
    G --> G2["task · authorship · policy · interceptor"]
...
```text
   cage
    |
    |-- THE DAILY FIVE  (shown at the front door)
    |     import   pull usage in
    |     setup    wire a project
    |     doctor   is capture healthy?
    |     query    how is a number computed?
    |     clean    prune stale state/ (STATE-RETENTION)
    |
    |-- FIVE GROUPS  (named, not expanded)
    |     insights  5 derived views
    |     task | authorship | policy | interceptor
    |
...
| reclaim disk space in `.cage/state/` | `cage clean --apply` |
...
4 hidden plumbing commands — resolving to **23 addressable commands**. The front door
(`cage --help`) shows only the curated five plus the group names; this section is the
whole surface.
...
- **`cage <group> <command>`** — every group (`insights`, `task`, `authorship`,
  `policy`, `interceptor`) is a real subparser; each command accepts `--help` and owns
  its own flags. This was not always true — see [Known gaps](#known-gaps).
...
They are global too, so `cage --no-import insights chats` works. The fleet study's
bundle export carried `--no-import` with a *different* meaning (skip the bundle's own
import-first refresh); it went with the whole study in v0.51, so the flag now has one
meaning everywhere.

### Export flags

Every `cage insights` view plus the one other report-shaped view,
`cage authorship summary`, takes these two, so
...
([ADR-KIRO](0006_kiro.md)); the summary
...
| `--hooks` | also wire the opt-in **L1** lifecycle hooks (agent identity at capture, auto task-close). OFF by default; re-running without it **removes** them. CLI sessions only — hooks do not fire under a VS Code extension |
...
Prune stale `.cage/state/` debris over the closed allowlist ([ADR-CLEANUP](0011_cleanup.md)).
Dry-run table by default (house pattern); `--apply` executes. Runs regardless of
`[cleanup] enabled` — that switch gates only the automatic reminder (`cleanup.maybe_run`),
never a command you typed.

| Flag | Meaning |
|---|---|
| `--apply` | actually prune (default: dry-run table) |
| `--days N` | override `[cleanup] days` for this run |
| `--json` | machine-readable output |
...
Who wrote which files in which commit (ADR-AUTHORSHIP). Its own closed enums, separate from
...
Named `interceptor`, not `graphify`, on purpose: [ADR-GRAPHIFY](0008_graphify.md) §2
...
| Command | What it does | Flags |
|---|---|---|
| `cage mcp` | run the MCP server over stdio — 1 read tool + exactly one write tool (`cage_task_outcome`). Wired automatically by `cage setup` | — |
...
**All 29 addressable commands, each with a runnable line.** Examples are inline code
...
most convincing disguise. Written this way, every command and flag named below is
checked to **exist**: `tests/test_cli_reference.py` walks subparser dispatch and the
declared flag set for each one. It does not call `parse_args`, so a missing required
positional is not caught here — see [Known gaps](#known-gaps).
...
| clean | `cage clean` | dry-run: what's stale in `.cage/state/`, nothing touched |
| clean | `cage clean --apply` | actually prune it (ADR-CLEANUP) |
| clean | `cage clean --days 30 --apply` | override the retention window for this run |
...
### `cage task` · `cage authorship` · `cage policy`
...
| `matrix` · `roi` · `verdict` · `budget` · `forecast` · `regression` · `recommend` · `prices` · `quality` | **removed outright in v0.50** with the money subsystem ([ADR-LAWS](0001_laws.md) Law 5) |
...
| `export` · `watch` · `serve` · `proxy` · `meter` | **removed outright in v0.50** — the whole `data` group. Capture is pull-based: use `cage import`. There is no bundle format at all any more (see `study` below) |
| `study` | **removed outright in v0.51** (STUDY-CUT) — the whole fleet study, not just its six actions: phase markers, the opaque machine id and the `machine` row field it stamped, the one-file zip bundle, and the `cage import BUNDLE` merge that read it. Nothing replaces it; cage no longer aggregates across machines |
| `graphify` (`data graphify`) | `cage interceptor graphify` — the `data` group went, the interceptor door it fronted did not (PG, v0.51) |
| `cleanup` (`data cleanup`) | `cage clean` — its own top-level verb, not re-grouped under a revived `data` ([ADR-CLEANUP](0011_cleanup.md), STATE-RETENTION) |
...
**`matrix` is re-ratified, not yet returned.** [ADR-MATRIX](0014_matrix.md) (2026-08-15)
ratifies a new insights view — tool-combination cost per closed task, tokens-only —
under the same top-level word this table's `matrix` row names as removed with the money
subsystem. It is **not the same command**: no USD, no `cage/compare.py` code path, built
on `taskgroup.join` rather than the deleted `prices.call_usd` roll-up. **Not built**
(`cage/matrixview.py` does not exist) — tracked as MATRIX-BUILD in
[OPEN-WORK.md](../../work/OPEN-WORK.md). Until it ships it stays out of the §2 reference
tables above and the output-block gallery below, both bidirectionally checked against
`cli.build_parser()` — naming an unparseable invocation there would turn the suite red
...
1. ~~`data migrate-savings` missing from the front door~~ — fixed 2026-08-03
   (CLI-GAPS(a)). The front door stays gated bidirectionally against the live parser,
   which is what makes a deletion this size safe.
2. ~~`prices`/`study`/`policy` take their action as a positional choice~~ — converted to
   <!-- `prices` and `study` have both since been removed; `policy` is the survivor. -->
...
**Fifteen blocks covering every view that prints.** The Examples section above answers
*what do I type*; this one answers *what comes back* — and the second question is where
this surface keeps its promises. An invocation tells a reader nothing about the `—` that
stands where a number would be, that the Σ row refuses alongside its rows, or that the
unknown-rate is printed before the counts rather than under them. Those are decisions,
they are the ones most often eroded by a well-meaning renderer change, and prose about
them is weaker evidence than the bytes.

**Every block carries its provenance class, because the two are not equally trustworthy:**

- **GATED** — the block is the byte-exact body of a golden under
  [`tests/fixtures/goldens/`](../../tests/fixtures/goldens/), rendered from a pinned seed
  in [`tests/goldenseed.py`](../../tests/goldenseed.py) and now asserted twice:
  [`tests/test_output_spec.py`](../../tests/test_output_spec.py) asserts the live CLI
  still prints it, and
  [`tests/test_adr_output_blocks.py`](../../tests/test_adr_output_blocks.py) asserts this
  record still carries it. A renderer change that skips either surface is red.
- **CAPTURED** — real stdout, pasted from a seeded run. Its **invocation line** is
  gated against the live parser; its **body is not**. `cage doctor` is the one
  **permanent** occupant of this class: it probes the local filesystem, so a pinned
  copy of it would be a claim about your machine that cage never made — the same call
  `test_output_spec.py` made for the fleet study's removed enrollment verb.
  **ADR-OUTPUT-GOLDENS**
  (2026-08-15, closed): the other seven blocks this class used to hold — bare `cage`,
  `import`, `setup --status`, `query saved`, `insights graphify`, `insights why`,
  `authorship origin` — are now GATED; only `doctor` remains here, by design rather
  than debt.

**The fences below are invisible to the reference gate, and that is a hazard worth
naming.** `test_cli_reference.py` strips fenced blocks before its dead-verb scan — which
is exactly why the examples above are inline spans — so a verb removed tomorrow could sit
in an output block here forever without turning anything red. `test_adr_output_blocks.py`
closes that hole from the other side: it reads the `$ cage …` line of every block in this
section and resolves it against `cli.build_parser()`, the same walk the reference gate
uses. **Block bodies are still unchecked prose to both gates** — a stale figure inside a
CAPTURED body is invisible, which is the whole argument for moving each one to GATED.
...
<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/H1.txt`](../../tests/fixtures/goldens/H1.txt)</sub>

#### `cage import` — the sweep, with nothing wired yet

The empty case is the one worth printing: three agents, zero calls, and a warning that
names the cause rather than a silent success. `imported 0 call(s)` and *nothing is
configured* are different claims and both appear.

```text
$ cage import
⚠ no [sources] in cage.toml — cage is capturing nothing. Run `cage setup` (or `cage setup --sync-sources`) to materialize the defaults.
✔ claude: imported 0 call(s) from 0 file(s).
✔ copilot: imported 0 call(s) from 0 file(s).
✔ kiro: imported 0 call(s) from 0 file(s).
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/H2.txt`](../../tests/fixtures/goldens/H2.txt)</sub>

#### `cage setup --status` — read-only wiring report
...
<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/W1.txt`](../../tests/fixtures/goldens/W1.txt)</sub>

#### `cage doctor` — abridged

**Machine-dependent by construction, so it can only ever be CAPTURED**, and abridged
here to the checks that carry a decision — the full run prints 45 lines. `…` marks every
cut. Note the shape rather than the values: `·` is *optional wiring absent*, `✔` is
*checked and healthy*, `✗` is *broken and named*, and the closing line grades the whole
run rather than leaving a reader to count marks.

```text
$ cage doctor
  · tool         `cage` not on PATH (running, but not globally callable)
  ✔ footprint    active ledger: project (.cage/) → <project>/.cage
  ✔ policy       config: cage.toml loads
  …
  ✔ claude-metrics claude-metrics (.cage/ledger/claude/): none yet — run `cage import --agent claude`
  …
  ✔ state        state/: 1 file(s), 0 KB · cleanup on (90d) · nothing stale
  …
  ✔ wiring       all wired commands name live verbs
  …
  ✔ integrity    integrity: nothing chained yet — the chain advances on `cage import`
  · interceptor  graphify interceptor not installed (ok if you don't use graphify)
  ✔ path-interceptor no `graphify` on PATH — nothing to intercept
  …
  · receipts     receipts: 0 — attribution has no data yet; savings are recorded by `cage interceptor graphify -- …`, the fux/compressor shims, and the response cache
  ✔ ledger       ledger write+read round-trip OK (saved derived correctly)
  …
· Cage works; some optional wiring is missing (see ·).
```

<sub>**CAPTURED** · real stdout from a seeded run, 2026-08-14 · the body is **not gated**, only the invocation line is. Cut for length, not for comfort. **DOCTOR-DEAD-VERBS** (2026-08-15, fixed): the `metering` check printed the v0.50-deleted **data export** and **data watch** verbs by name in its "last import" and pull-based-capture lines; both now name only `cage import`.</sub>

#### `cage query saved` — one explainer topic

```text
$ cage query saved
saved · the tokens a tool kept out of the prompt (GROSS)
  formula:  saved = raw_alternative − actual   (in the receipt's own unit — usually tokens)
  ...
  method:   inherits the receipt's method — measured only if the tool truly measured it.
  code:     cage/savings.py · cage/graphifychat.py · cage/schema.py
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/H4.txt`](../../tests/fixtures/goldens/H4.txt)</sub>
...
-------------------  -------  -------  -----  ---------  ---------  -----------  ----------  -------  ------
fix the flaky test   claude   —            1    900,000          0            0      60,000        —     62%
refactor the parser  copilot  vscode       1    196,801          0            0       9,621        —       —

· claude credits: — (Claude Code records no credit unit on disk)
· agent% is the share of evidenced lines in files this chat touched — not a share of the chat's work; lines in files no chat proposed belong to no chat (`cage query agent-authorship`)
· agent% `—` for copilot: its stores record usage and prompts, not the text of an edit · kiro: its usage log records token counts only, with no tool-input payload
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/I10a.txt`](../../tests/fixtures/goldens/I10a.txt)</sub>

The two `—` cells are the record's honesty rule rendered: `credits` is absent because
Claude Code writes no credit unit to disk, and copilot's `agent%` is absent because its
stores hold usage and prompts but never the text of an edit. Neither is `0`.


#### `cage insights chats` — the empty ledger

An empty view names the two commands that would fill it. A bare table with no rows
would be indistinguishable from a broken import.
...
<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/I10c.txt`](../../tests/fixtures/goldens/I10c.txt)</sub>

#### `cage insights graphify`

```text
$ cage insights graphify
Graphify per-chat usage & GROSS saving

chat   agent   surface  gfx uses  tokens  without gfx   saved  saved%
-----  ------  -------  --------  ------  -----------  ------  ------
s_gc1  claude  cli             1  21,400       64,000  42,600     67%
s_gc2  claude  vscode          1   5,400       13,200   7,800     59%
...
· tokens is the chat's recorded tokens_in + tokens_out — the WITH-graphify world (cached/cache-write excluded); without gfx = tokens + Σsaved is the MODELED without-graphify counterfactual — it mixes a graphifymeter.toks estimate against metered tokens, so saved% is modeled throughout (`cage query graphify-chats`)
· method: modeled (confidence 0.60, worst case across the receipts shown)
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/I11.txt`](../../tests/fixtures/goldens/I11.txt)</sub>

#### `cage insights commits`

**No `$` appears anywhere in this table and none ever will** — the standing v1 veto,
pinned by an assertion in `tests/test_output_spec.py`, not by intent.

```text
$ cage insights commits
Commits · main · 3 commits

commit   date · time    tok in  tok out  cache r  cache w  human hrs    agent / human~ / unattr / unkn
-------  -------------  ------  -------  -------  -------  ---------  --------------------------------
381dcae  07-01 · 11:30       —        —        —        —          —           0% /   0% / 100% /   0%
9e41bcc  07-01 · 10:00       —        —        —        —          —          50% /  33% /  17% /   0%
77e8c6c  07-01 · 09:00       —        —        —        —          —           0% /   0% / 100% /   0%
───────
Σ 3      —                   —        —        —        —          —          38% /  25% /  38% /   0%

· split = share of CLASSIFIED added lines. agent = matched an agent's recorded
  proposal (direct); human~ = added in a file that session proposed but matching
  nothing (ESTIMATED residual); unattr = added in a file no session proposed —
  a person, a vendored tree or generated output, cage does not guess which;
  unkn = below the content gate or binary. Never redistributed, never a score.
· human hrs marked ~ are ESTIMATED: commit wall-clock minus the agent's turn-span,
  floored at 0 and refused past [authorship] max_est_gap = 4h. An inference, not a
  measurement. * = attested via `cage task time` — always wins over the estimate.
· tokens are MEASURED; placing a call on a commit is MODELED (task-id join first,
  commit-window fallback). No USD on this surface, by design.
· not window-joinable: copilot/cli: one shutdown timestamp per session — every call in a session shares it, so a window join would dump the whole session on one commit · kiro: its rows carry the IMPORT time, not the call time — the log has no per-call timestamp to place
· not line-matchable: copilot: its stores record usage and prompts, not the text of an edit · kiro: its usage log records token counts only, with no tool-input payload — their commits show as unattr, never 0%
· 3 commit(s) unattributed — no joinable call. Tokens render '—', never 0:
  cage has no evidence they cost nothing.
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/A1.txt`](../../tests/fixtures/goldens/A1.txt)</sub>

#### `cage insights commit SHA`

```text
$ cage insights commit 9e41bcc
commit 9e41bcc · 2026-07-01 10:00 · main · claude-code · 1 session(s) recorded · no call joined

  tokens     —  no joinable call — never 0
  origin     agent · confidence 0.67 (transcript) — human only by attestation, unknown by absence
  lines      +6 / −0 total
             agent             3    line-match vs the session's recorded edit blocks
             human~            2    residual in files the session proposed (estimated)
             unattr            1    in files no session proposed — cage does not guess
             unkn              0    below the content gate / binary
  suggested  4 line(s) across 1 session(s)
  kept       3 verbatim · 1 landed-modified · 0 dropped
  time       wall 1h00m · agent — · human —

file            +  −  agent  human~  unattr  unkn
--------------  -  -  -----  ------  ------  ----
mod.py          5  0      3       2       0     0
generated.json  1  0      0       0       1     0
  ───────────────────────────────────────────────────
  Σ suggested 4 · kept 3 — counts, not a score
...
· not window-joinable: copilot/cli: one shutdown timestamp per session — every call in a session shares it, so a window join would dump the whole session on one commit · kiro: its rows carry the IMPORT time, not the call time — the log has no per-call timestamp to place
· not line-matchable: copilot: its stores record usage and prompts, not the text of an edit · kiro: its usage log records token counts only, with no tool-input payload — their commits show as unattr, never 0%
· 1 commit(s) unattributed — no joinable call. Tokens render '—', never 0:
  cage has no evidence they cost nothing.
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/A3.txt`](../../tests/fixtures/goldens/A3.txt)</sub>

#### `cage insights why CALL_ID`

Full provenance for one call: the call, then every receipt filed against it with the
method that produced each — `measured` and `modeled` side by side in one table, never
folded into a single number.

```text
$ cage insights why c_why1
c_why1  ·  2026-07-01T12:00:00Z
  task     fix-handover-bug    route chat    agent claude
  model    anthropic/claude-sonnet-4-6
  tokens   8,600 in (0 cached) / 1,500 out
  latency  0 ms    ok=True

Receipts
tool        unit    raw alt  actual   saved  method
----------  ------  -------  ------  ------  --------
graphify    tokens   30,000   3,000  27,000  modeled
fux         tokens    8,000   1,600   6,400  modeled
compressor  tokens   10,000   2,000   8,000  measured
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/I12.txt`](../../tests/fixtures/goldens/I12.txt)</sub>
...
**The unknown-rate is the headline, not a footnote** — `test_output_spec.py` asserts
`UNKNOWN` appears before `recorded` in the byte stream.

```text
$ cage authorship summary
Authorship · 3 commit(s)

  UNKNOWN     2 of 3 commit(s) (67%) have no authorship row at all
              — unknown by ABSENCE, never a stored row
  recorded    1 commit(s) · 1 row(s)
...
transcript     1
  ...
  suggested 4 · kept 3 verbatim · 1 landed-modified · 0 dropped
...
<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/A4.txt`](../../tests/fixtures/goldens/A4.txt)</sub>

#### `cage authorship origin SHA`

```text
$ cage authorship origin 9e41bcc
9e41bcc  ·  origin=agent  ·  confidence=0.6667

Provenance rows
agent        files   method      origin  confidence
-----------  ------  ----------  ------  ----------
claude-code  mod.py  transcript  agent   0.67
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/A5.txt`](../../tests/fixtures/goldens/A5.txt)</sub>
...
```text
$ cage policy diff
policy sync — bundled v0.27.0 vs project v0.19.0

· 8 project keys equal to the bundle — in sync
...
· bundled policy defaults are newer (v0.27.0 > v0.19.0) — run 'cage policy sync'
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/P5.txt`](../../tests/fixtures/goldens/P5.txt)</sub>

---

### The marker verbs — one line, no block

A verb that records a fact rather than rendering a view owes its reader one line of
confirmation and nothing more. Each is real stdout from the same seeded run.

| command | what it prints |
|---|---|
| `cage task outcome` | `✔ recorded 't_0042' as ok (label: refactor).` |
| `cage task time` | `✔ attested 45m of human time on 't_0042'.` — then a second line saying the hours stay invisible until the task is closed |
| `cage authorship verify` | `cage authorship verify: 0 warning(s) — report-only, never fails the build.` |
| `cage demo` | `✔ Seeded the §4.4 worked example (task 'fix-handover-bug', call c_…).` plus the two reads to run next |

`cage mcp`, `cage hook` and `cage interceptor graphify` print nothing to a human by
design — they answer a machine over stdio, a host over an exit code, and a shim over a
receipt respectively. `cage debug` prints the metadata-only log, which is empty unless
`CAGE_DEBUG=1` was set for the run that produced it.
...
A second gate, [`tests/test_adr_output_blocks.py`](../../tests/test_adr_output_blocks.py),
owns [What the output looks like](#what-the-output-looks-like): every GATED block
matches its golden byte-for-byte, every block's `$ cage …` line resolves in the live
parser (fences are invisible to the reference gate, so this is the only thing standing
between a deleted verb and a permanent home in an output block), and every block
declares which of the two classes it is.
