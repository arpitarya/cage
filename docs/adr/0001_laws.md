...
**no exceptions**: every configured source, including Kiro's IDE log, captures into the
one ledger this resolution names. **`project` is a derived view, not a capture scope** — a
`project` field stamped where the log exposes a cwd, not a per-project ledger; a source
with no project dimension of its own (Kiro's IDE log) still lands in the run's one active
ledger, it simply carries no `project` field on the row ([ADR-KIRO](0006_kiro.md)).
*Ratified as* archived ADR 0002 — **named, not cited**. Kiro's IDE log was carved out as a
routed exception from 2026-08-01 to 2026-08-15, ratified then reversed —
