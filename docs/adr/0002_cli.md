---
adr: cli
status: current as of 2026-08-14 · 14 top-level entries · 28 addressable commands · an example per command AND rendered output for every printing view · gated bidirectionally against the live parser
...
|---|---|
| `BUNDLE …` | study bundle zip(s) from `cage study export` — merged by row identity, idempotent (ADR-CONSUMERS) |
...
The P5 fleet study (ADR-CONSUMERS). Opaque random machine id, **opt-in by enrollment** — an
...
- **CAPTURED** — real stdout, pasted from a seeded run on 2026-08-14. Its **invocation
  line** is gated against the live parser; its **body is not**. This class exists because
  some output cannot honestly be a byte golden: `cage doctor` probes the local filesystem,
  so a pinned copy of it would be a claim about your machine that cage never made — the
  same call `test_output_spec.py` already makes for `cage study join`. The rest of this
  class is a coverage debt, not a design, and it is filed as **ADR-OUTPUT-GOLDENS** in
  [OPEN-WORK.md](../../work/OPEN-WORK.md).
...
<sub>**CAPTURED** · real stdout from a seeded run, 2026-08-14 · the body is **not gated**, only the invocation line is.</sub>
...
<sub>**CAPTURED** · real stdout from a seeded run, 2026-08-14 · the body is **not gated**, only the invocation line is. Cut for length, not for comfort — one cut hides a live defect, **DOCTOR-DEAD-VERBS** in [OPEN-WORK.md](../../work/OPEN-WORK.md): the `metering` and `timeline` checks still print the v0.50-deleted **data export** and **data watch** verbs by name. Reproducing them here would document a bug as a contract.</sub>
...
----  ------  -------  --------  ------  -----------  ------  ------
s_g1  claude  cli             2  21,400       64,000  42,600     67%
s_g2  claude  vscode          1   5,400       13,200   7,800     59%
...
```text
$ cage insights why c_1a001db8fc6dff649a2
c_1a001db8fc6dff649a2  ·  2026-06-01T12:00:00Z
  task     fix-handover-bug    route code-edit    agent claude-code
...
### `cage study report` · `cage policy diff`

#### `cage study report`

Coverage first, delta second — a gap day is printed beside the machine that has it,
because a delta computed over a phase with holes in it is the failure this view exists to
make visible.
...
coverage (days with rows — gaps kill studies, so they print first):
  machine m_01aa000000000000
    baseline     5 day(s) · agents: claude-code · ⚠ gap days: 2026-07-06
    plugin       5 day(s) · agents: claude-code
...
pooled machine-days per phase (measured):
  baseline     n=30 days · median 245,000 tok (IQR 230,000–260,000)
  plugin       n=25 days · median 146,000 tok (IQR 140,000–152,000)
```

<sub>**GATED** · byte-exact from [`tests/fixtures/goldens/S3.txt`](../../tests/fixtures/goldens/S3.txt)</sub>
...
| `cage study id` | `not enrolled — cage study join <phase> (or start) generates the opaque machine id` |
| `cage study start` · `cage study stop` | `phase 'baseline' started` · `phase stopped` — deliberately never byte-pinned: `join` runs wiring and doctor, whose output is machine-dependent |
