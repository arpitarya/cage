---
adr: kiro
status: current as of 2026-08-15 · credits captured and measured, stored in `ledger/kiro/` (P2, v0.51) · the IDE store's rows moved there too (KIRO-CALLS-LEG, ratified) · IDE routing to a machine-only ledger REVERSED, see ADR-LEDGER · NO token spine · CLI token-slot upgrade-watch armed · `devdata.sqlite` parser removed, DEVDATA-CUT
...
summed into any total — so cage reports credits everywhere it prices, refuses a total for
kiro tokens, and says `—` where a number would be fiction. Since LEDGER-READ-SURFACE
(2026-08-15) `insights chats` is the one exception: it shows kiro's real, coarse IDE token
counts **per chat**, never folded into a total.
...
- **A standing upgrade-watch is armed:** the day Kiro's own CLI store starts persisting
  non-NULL per-turn tokens, cage captures them with **zero code change**, and `cage doctor`
  announces the flip. (A second watch, for an IDE-side `devdata.sqlite` store, was retired
  2026-08-15 — DEVDATA-CUT — after never once observing that store on a probed install;
  see the Veto condition for what would bring a watch like it back.)
...
flowchart TD
    ...
    subgraph ide["Kiro IDE — knows nothing"]
        ...
        SQ["dev_data/devdata.sqlite<br/>never observed on any probed install<br/>parser + probe REMOVED 2026-08-15 (DEVDATA-CUT)"]
    end
    ...
    MC -->|"LEDGER-READ-SURFACE (2026-08-15)<br/>real per-chat tokens, never a total"| V
```
...
```text
  ...
  KIRO IDE  -- knows NOTHING (no project, no session, no timestamp) --------------
    dev_data/tokens_generated.jsonl  -- the only IDE store that exists
        +-- ledger/kiro/ ----------> ide-log rows, THIS RUN'S active ledger
                                     same resolver as every other agent (ADR-LEDGER)
                                     +-- insights chats (LEDGER-READ-SURFACE, 2026-08-15)
                                         real per-chat tokens shown, never a total
    (dev_data/devdata.sqlite never observed on any probed install; its parser and
     probe were removed 2026-08-15 — DEVDATA-CUT — rather than kept armed for a
     store that has never once appeared)
...
| IDE tokens | a coarse global log, captured into whichever ledger is active for the run, shown per-chat in `insights chats` (LEDGER-READ-SURFACE) | vendor-recorded, coarse — no per-turn precision regardless of ledger; never summed into a total |
...
- **`devdata.sqlite` does not exist.** Through v0.50, `ABSENT_SPINES` pointed at it — a
  ...
  field probe settled it: `dev_data/` holds only `tokens_generated.jsonl`. Its parser and
  probe stayed **kept-but-armed** for a few weeks after that answer, then were removed
  outright 2026-08-15 (DEVDATA-CUT) — an unreachable store is not worth a live reader.
...
- **`SPEND_SOURCES["kiro"] = ()`** and `ABSENT_SPINES["kiro"]` carries the reason. Every
  `spend()`-backed view renders `—` **with that sentence**, never a `0`.
  **Two absence tables, deliberately not merged** — `ledger.ABSENT_SPINES["kiro"]` is the
  narrow one-clause *"no IDE token store on this install"*, read by the `spend()`-backed
  views; `units.ABSENT["kiro"][TOKENS]` is a **second, wider** sentence read by
  `insights chats` through `chats._unit_absence_notes`. **It is the `units.ABSENT` one that
  was widened in P2**, not `ABSENT_SPINES`, which is unchanged. The widening was needed
  because one clause named one surface and one cause, and a reader could fairly conclude
  the CLI had tokens. **Both** surfaces lack them, for two different vendor reasons — the
  IDE ships no store at all; the CLI's token columns exist and are still null (2.16.0, an
  upgrade-watch rather than a permanent absence). One clause could not carry that
  distinction, and it is rendered verbatim to users.
  Through 2026-08-15, `transcript.parse_kiro_ide_metrics` was kept for the day a Kiro
  ships the `devdata.sqlite` store, with `cage doctor`'s kiro-IDE check distinguishing
  **db absent / table missing / column drift** so the flip would announce itself. That
  reader and probe were **removed** (DEVDATA-CUT, 2026-08-15): the store had never once
  been observed on a probed install, so there was nothing live left to distinguish. Only
  the CLI's `cli-turn` upgrade-watch (NULL token slots) remains armed.
...
> `make_kiro_metric` stamps it the same way. Both consequences P5 named survive in reduced
> form — the routing decision still has rows to route, and the CLI's `cli-turn`
> upgrade-watch still has arriving rows. (The `devdata.sqlite`-arrival watch this passage
> originally also named was retired 2026-08-15 — DEVDATA-CUT — the store never arrived on
> any probed install; `cage doctor` now counts `ide-log` alone, the one real IDE grain.)
>
...
- **Kiro contributes no tokens to any total.** That is the decision, stated, not a bug to
  fix. `units.ABSENT["kiro"][TOKENS]` carries the reason to every `spend()`-backed
  renderer (report, `insights commits`). Since LEDGER-READ-SURFACE (2026-08-15) that
  reason is no longer unconditional: `insights chats` reads `ledger.kiro_metrics()`
  directly — a second, narrower surface `spend()` never touches — and shows real
  per-chat tokens there; `chats.py`'s `_unit_absence_notes` suppresses the "no tokens"
  line specifically when a shown kiro row carries them, so the two claims never sit on
  screen contradicting each other. The total itself is still refused everywhere.
...
> A fourth grain, `ide` (IDE `dev_data/devdata.sqlite`), lived in this table through
> 2026-08-15: the same counter as `ide-log` plus a `timestamp` and a cursorable `id`, kept
> as the manifest-eligible alternate half of an `ide`/`ide-log` pair pending a store that
> never shipped. Removed outright (DEVDATA-CUT) rather than kept armed — `ide-log` is now
> kiro IDE's only grain, with nothing waiting to be flipped in.

Parsers: `transcript.parse_kiro_ide_log_metrics` (the live IDE leg) ·
`parse_kiro_cli_credits` · `parse_kiro_cli_metrics` ·
`parse_kiro_cli_tool_runs` (**the one carve-out**) ·
...
Health: `cage doctor`'s `kiro-metrics` check names per-source coverage and the CLI
`cli-turn` upgrade-watch state (armed vs. tripped). `cage query kiro-metrics` explains it.
Routing since ADR-LEDGER (2026-08-15): `ide-log` rows capture into this run's one
...
> **Vendor-recorded** = quoted verbatim from a Kiro store; **cage-derived** = computed,
> counted, or stamped by cage — never a literal field on disk. Fields the vendor exposes
> but that are NULL on every real store probed (the CLI token slots — see *Known gaps*)
> are not listed here as captured data.

| Data point | Source file (on disk) | Field/key read | Extracted by | Vendor-recorded / cage-derived |
|---|---|---|---|---|
| Input tokens (IDE) | `dev_data/tokens_generated.jsonl` | `promptTokens` | `transcript.parse_kiro_ide_log_metrics` | vendor-recorded |
| Output tokens (IDE) | same | `generatedTokens` | same | vendor-recorded — usually 0 |
| Model (IDE) | same | `model` | same | vendor-recorded — often the generic `"agent"` |
| Provider (IDE) | same | `provider` | same | vendor-recorded |
| Row id (IDE) | same | line index + `sha1(line)[:8]` | same | **cage-derived** — no id field in the file |
| Timestamp (IDE) | — | no `ts` field in the file | `schema.make_kiro_metric` stamps import time | **cage-derived** — not read from the source |
| Credits per conversation (CLI) | `kiro-cli/data.sqlite3` → `conversations_v2.value` | `user_turn_metadata.usage_info[]` where `unit` starts `"credit"` | `transcript._kiro_cli_credit_row` | vendor-recorded per-value; the sum is cage's aggregation |
| Context % used (CLI) | same | `history[].request_metadata.context_usage_percentage` (last non-null) | `_kiro_cli_credit_row` / `parse_kiro_cli_metrics` | vendor-recorded |
| Turn count (CLI) | same | `len(history)` | `_kiro_cli_credit_row` | **cage-derived** — array length, not a stored field |
| Model (CLI, conversation) | same | `model_info.model_id` (fallback `model_name`) | `_kiro_cli_credit_row` | vendor-recorded |
| Conversation timestamp (CLI) | `conversations_v2` row | `updated_at` column (epoch ms → ISO) | `_kiro_cli_credit_row` (`_epoch_ms_iso`) | vendor-recorded, format-converted |
| Project label (CLI) | `conversations_v2` row | `key` column, basename only | `_kiro_cli_credit_row` | **cage-derived** — basename extraction, PII guard |
| Chunks / tool-use count (CLI, per turn) | same conversation JSON | `len(request_metadata.time_between_chunks)` / `len(request_metadata.tool_use_ids_and_names)` | `parse_kiro_cli_metrics` | **cage-derived** — array lengths |
| Prompt / response bytes (CLI, per turn) | same | `request_metadata.user_prompt_length` / `request_metadata.response_size` | `parse_kiro_cli_metrics` | vendor-recorded |
| Turn message id / timestamp (CLI) | same | `request_metadata.message_id` / `stream_end_timestamp_ms` (fallback `request_start_timestamp_ms`) | `parse_kiro_cli_metrics` (`_first_int`) | vendor-recorded |
| Graphify savings inputs (CLI, transient) | same | `history[].assistant.ToolUse.tool_uses[].args.command` + `history[].user.content.ToolUseResults.tool_use_results[].content[].Json.stdout` | `_kiro_cli_tool_runs` / `parse_kiro_cli_tool_runs` | **cage-derived** — hashed/counted in memory; command and stdout never persisted |
...
- **Kiro has NO token spine, and cage says so rather than showing a zero.**
  ...
  IDE token store on this install"* — which every `spend()`-backed view (report,
  `insights commits`) renders as `—`. This is unchanged by LEDGER-READ-SURFACE below:
  `insights chats` reads `ledger.kiro_metrics()` directly, never through `spend()`, so
  kiro still contributes zero tokens to any cross-agent total.
...
- ~~**No read surface for `.cage/ledger/kiro/`**~~ **Closed 2026-08-15
  (LEDGER-READ-SURFACE).** `insights chats` now reads `ledger.kiro_metrics()`'s
  IDE-sourced rows directly and renders them as their own chat row (constant-session
  collapse, `KIRO_IDE_LABEL`), mirroring the CHATS-CREDITS pattern already documented
  above — real, coarse tokens shown, credits dashed (the IDE store has none), never
  joined into `spend()`. A dedicated `cage insights kiro` view is still not built; the
  CSV export that was also parked went with `cage data`, deleted by SURFACE-CUT.
...
- **One real-store probe still pending** — whether IDE session JSONs embed per-message
  ...
  does not exist**, and the parser built against that answer was removed 2026-08-15
  (DEVDATA-CUT) rather than kept armed for a store that never appeared.
...
- **LEDGER-READ-SURFACE, closed 2026-08-15** (same day, directed by Arpit — "kiro cli
  and ide both should print on cage insights chats"): `insights chats` reads kiro-IDE
  metrics rows directly, mirroring CHATS-CREDITS; the column contract lives in
  [docs/FORMULAS.md](../FORMULAS.md) §2.13, the invariant it does not touch (kiro
  still contributes zero tokens to `spend()`) is pinned in `tests/test_kiro_metrics.py`
  and `tests/test_chats.py`.
...
2. **The absent token spine** reopens the moment kiro IDE ships any summable store — a
   `devdata.sqlite` reappearing, or some other IDE store carrying real per-call tokens.
   The 2026-08-15 removal (DEVDATA-CUT) means this is no longer a contained flip of a kept
   parser: the `devdata.sqlite` reader and its `cage doctor` three-way probe (db absent /
   table missing / column drift) were deleted, never observed live, so a future store
   would need a parser and probe **rebuilt from a fresh field probe of its real schema**,
   not resurrected from memory of the old one.
...
- **One real-store probe still pending:** whether IDE session JSONs embed per-message
  usage. Whatever the answer, it would need a parser built fresh against a real probed
  schema — the `devdata.sqlite` parser this ADR once kept on standby for a similar
  question was removed 2026-08-15 (DEVDATA-CUT) rather than reused speculatively.
