> **ARCHIVED 2026-08-12 — `docs/open/` no longer exists.** The whole queue was closed
> wholesale on Arpit's explicit instruction at v0.49.0; every item file moved to
> `docs/archive/v0.49-*.item.md` with its own closure header. This was the directory's
> own README, describing a maintenance law that no longer has a directory to govern.
> **History, never spec.**
...
| [CONSTRAINTS.md](v0.49-open-queue-constraints.md) | rules that outlive their originating item — **not** open work |
