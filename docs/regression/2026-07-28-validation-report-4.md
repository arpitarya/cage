...
> **⛔ SUPERSEDED — split into [run-002 (pre-fix)](2026-07-28-validation-run-002.md) + [run-003 (post-fix)](2026-07-28-validation-run-003.md).**
> This layered file answered three questions in one document (pre-fix baseline ·
> post-fix re-run · cross-run findings) and could be misread mid-file — line 92 reads
> `227,298 · 189,788 ❌` while line 14 reads `8/8 exactly 227,298`, both true of
> *different* runs. It is preserved unedited below the marker as published evidence;
> **do not cite it as current.** Read the two per-run reports, the dated finding docs,
> or `cage-lab/golden/findings/HISTORY.md` for cross-run movement.
> (Body and its sha256 below are unchanged — this banner sits above the hashed range.)

**Report sha256 (body below the marker):** `e22e6eeac5ea65cb10bbb39ba3f7575e47f8bb84fe6fa3e49525a6a201b1a8d9`
...
list: `cage/docs/regression/2026-07-28-capture-precision-fixes.md`.

---

**Status:** scripted cells V1–V5b **CLOSED green** — see the post-fix re-run at
the [top of this report](#re-run-results-post-fix-2026-07-28--capture-precision-cycle)
(copilot 8/8 exact, kiro credits, claude byte-identical). **Open:** manual cells
V6–V11 (`manual/vscode-checklist.md`, Arpit's hands) + Phase 2. Everything from §1
down is the **pre-fix BASELINE**, preserved as evidence and superseded by the
re-run above — see the banners on §1/§2/§3.1. **STOP point reached** — the manual
sweep and Phase 2 await Arpit's go.
...
> **⚠ BASELINE (pre-fix, 2026-07-28) — superseded by the [re-run at the top of this report](#re-run-results-post-fix-2026-07-28--capture-precision-cycle).**
> The ❌ cells below (V3/V4 check 5, V5/V5b checks 4–5) are the **pre-fix** evidence
> that the capture-precision fix was needed. They are preserved unedited, not current:
> the re-run shows V3/V4 copilot **8/8 exact** and V5/V5b capturing credit rows. Do
> not read the grid below as cage's current state.
...
> **⚠ BASELINE (pre-fix, 2026-07-28) — superseded by the [re-run at the top of this report](#re-run-results-post-fix-2026-07-28--capture-precision-cycle).**
> The undercounted copilot figures below (V3 `189,788`, V4 `191,414`) are the
> **pre-fix** numbers. They are the evidence the fix was needed and are preserved
> unedited. Post-fix, cage re-imports these same logs to **exactly** 227,298 (V3)
> and 233,675 (V4). Do not read the cage-side numbers below as current.
...
> **⚠ BASELINE (pre-fix, 2026-07-28) — FIXED; superseded by the [re-run at the top of this report](#re-run-results-post-fix-2026-07-28--capture-precision-cycle).**
> This finding describes the resumed-session undercount **as it existed pre-fix**.
> The capture-precision fix shipped 2026-07-28 (`cage/docs/regression/2026-07-28-capture-precision-fixes.md`):
> on re-seeing a session id cage now updates the row to the last cumulative
> shutdown, so the self-heal is proven (70,071 → 107,581 exact → +0). The mechanism
> below is retained as the *why*, not an open defect.
...
### 4.5 The IDE-store config nuance — RESOLVED (fixed 2026-07-28)

> **✅ RESOLVED (fixed 2026-07-28).** This LOW nuance predated the
> capture-precision surface-restamp fix (`cage/docs/regression/2026-07-28-capture-precision-fixes.md`,
> §3.5) and is now closed. Re-checked against fixed cage on 2026-07-28: a declared
> `[sources.kiro] surface="cli"` that collides with the built-in path now
> **upgrades the built-in — declared wins** — instead of being silently dropped.
> The original pre-fix observation is retained below as the *why*.

- **Pre-fix observation (superseded):** on the IDE token log the v0.36
  `surface="cli"` restamp worked on **distinct** content (rows → `surface="cli"`),
  but was silently lost when a declared source collided with a built-in source.
  Only bit in the overlap case; a genuinely distinct store wouldn't collide.
- **Fix + re-check (2026-07-28, fixed cage 0.36.0 working tree):** the collision is
  a source-**definition** collision by `(path, glob)` resolved in
  `paths.resolve_log_sources._emit` — a colliding declared `surface` now upgrades
  the built-in entry rather than dropping the whole entry. Verified two ways:
  (1) unit + import tests green —
  `tests/test_sources.py::test_declared_surface_wins_on_builtin_collision`
  (resolution: declared wins) and `::test_custom_tool_surface_restamps_alongside_agent`
  (import: rows actually carry `surface="cli"`);
  (2) a live reproduction of this exact scenario — declaring `[sources.kiro]
  surface="cli"` on the shipped built-in `tokens_generated.jsonl` path now resolves
  to one source with `surface='cli'` (pre-fix it resolved with `surface=''`). No
  longer LOW-open; a closed finding.
...
- **Gates for Phase 2 (Arpit), two cage findings that warrant `docs/regression/`
