...
P2 REV-HARDEN-P3 are closed**; P3–P6 remain (REV-HARDEN P4 mechanical, then its judgment
half, HR-COPILOT-JOIN, EXPORT-SCOPE). *(This line previously pointed at "tier 2 — four
...
Suite: **1562 pass / 0 fail / 11 skipped** — re-run on the maintainer's own venv
2026-08-11 (+3 CIGF-HERMETIC, +17 REV-HARDEN P3, over a **1542** baseline). *(The 1541
recorded in `README.md`, `CLAUDE.md` and `INTERVIEW.md` was itself one short of it.)*
...
| **REV-HARDEN P4** | **P0 · P1 · P2 · P3 all shipped** (P2's `gen_ai.system` item became OTEL-SEMCONV-PIN above; **P3 wiring hygiene closed 2026-08-11** — all five items, mutation-checked, and it found a **sixth** crash site the proposal missed plus a silent data-loss path on the *default* `cage setup`). **P4** durable joins & scale, seven items: full shas + prefix-match · `commitview` has no default window (one full `git show` per commit, O(n²)) · `_uncovered` judges coverage over other repos' edits · unthrottled session-end sweep · quoted-path line matching · Edit context lines counted as `suggested` · two `cigraphify` checks that assert nothing | [handoff](agent-lane-sweep.handoff.md) P3–P4, each independently landable. **The two halves must not be batched** — P4's judgment items change numbers already written to disk, and its sha rewrite lands LAST. ⚠️ The proposal's line numbers are badly stale (`tools/cigraphify.py:807,739` for a 303-line file) — locate by symbol. ⚠️ Its prefix-match symptom is backwards: `render_commit` takes `rows[0]` over an oldest-first sort, so it renders the **oldest** match; the missing *ambiguity refusal* is the real defect |
