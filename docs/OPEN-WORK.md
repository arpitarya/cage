...
**Next (agent lane):** the **agent-lane sweep** ([handoff](agent-lane-sweep.handoff.md)),
picked up 2026-08-11 — P0 needed no work (already released), **P1 CIGF-HERMETIC and
P2 REV-HARDEN-P3 and P3 REV-HARDEN-P4-mechanical are closed**; P4–P6 remain (P4's
judgment half, HR-COPILOT-JOIN, EXPORT-SCOPE). One fork was raised rather than decided
inside a fix commit: **COMMITS-WINDOW**. *(This line previously pointed at "tier 2 — four
fabricated numbers"; all of that shipped 2026-08-03.)*
...
Suite: **1583 pass / 0 fail / 11 skipped** — re-run on the maintainer's own venv
2026-08-11 (+3 CIGF-HERMETIC, +17 REV-HARDEN P3, +21 REV-HARDEN P4-mechanical, over a
**1542** baseline). *(The 1541 recorded in `README.md`, `CLAUDE.md` and `INTERVIEW.md`
was itself one short of it.)*
...
| **REV-HARDEN P4 (judgment half)** | **P0 · P1 · P2 · P3 shipped, and P4's *mechanical* half shipped 2026-08-11** — the two `cigraphify` checks that asserted nothing · `_uncovered`'s missing repo filter · the unthrottled session-end sweep · quoted-path line matching (which turned up **two** further defects: git's disambiguating tab on a spaced path, and a third C-quoting site stamping a mangled `scope` into the ledger). **What is LEFT is the judgment half, 3 items:** full shas + prefix-match with an ambiguity refusal · rename syntax (`old => new`) in numstat · Edit context lines counted as `suggested`. `commitview`'s window split out as **COMMITS-WINDOW** above | [handoff](agent-lane-sweep.handoff.md) P4. **Each changes numbers already on disk, so each is its own change and the sha rewrite lands LAST.** ⚠️ The proposal's prefix-match symptom is backwards: `render_commit` takes `rows[0]` over an oldest-first sort, so it renders the **oldest** match — the missing *ambiguity refusal* is the real defect. ⚠️ Its line numbers are badly stale |
| **EXPORT-SCOPE** | v0.48.0 wired `--export`/`--stamp` onto `cage report` + every `insights` leaf. **Three report-shaped views were left outside that scope line** and a user will reasonably expect them (verified against the live parser 2026-08-10): `cage authorship summary` · `cage study report` · `cage task quality` | wire with `cli._export_flags`, route the handler through `cliutil.emit(csv=…, root=…)`, and **add them to `EXPECTED_VIEWS` in `tests/test_view_export.py`** — that set is a gate that fails both ways. Picked up 2026-08-10 → [handoff](agent-lane-sweep.handoff.md) P6 |
| **COMMITS-WINDOW** | `cage insights commits` costs **one `git show` subprocess per commit in the WHOLE history** while the screen shows 20 rows — measured **6.4s for 20 rows on cage's own 123-commit repo** (1.2s with `--since 3d`). `constants.COMMITS_DEFAULT_ROWS`' comment claimed the row cap bounded cost; it does not — the cap is applied in `render_commits`, after every row is built. Comment corrected 2026-08-11; the O(n²) list scan and the silent-window cut were fixed in the same change | **decide, do not patch.** A default `--since` is the obvious fix and is **wrong** — it puts a wall clock in the default path (the goldens' fixed-date fixtures render differently next month) and contradicts INTERVIEW.md's *make a published window absolute, never relative*. Recommendation and the CSV-never-truncated sub-question in [compare](compare/commits-view-cost-bound.compare.md); verdict is Arpit's |
...
| # | what | next action |
|---|---|---|
| **HR-COPILOT-JOIN** | copilot-vscode has per-request timestamps but stamps **no `project`**, so every one of its calls is excluded as *unconfirmable* — the join is built and cannot fire for it | **unblocked 2026-08-02**; ⚠️ **the framing was wrong both ways** (verified 2026-08-10). The store *does* carry a cwd — two carriers, evidenced by this repo's own probe ([research](research/2026-08-07-graphify-store-evidence.md) §cwd) — but the **claude precedent does not transfer** and it does **not** join "for free": the per-request record carries no cwd at all, so the project must be resolved per *file* (`workspace.json`, full coverage) with the per-command `toolSpecificData.cwd.path` as fallback. Multi-root and `--path` runs need explicit fail-open. Picked up 2026-08-10 → [handoff](agent-lane-sweep.handoff.md) P5 |
...
| **OUTPUT-GROWTH** | `.cage/output/` has **no cleanup class by design** — cage never deletes an artifact it wrote (an as-of record is unrecoverable, the same standing `ledger/savings/` has), so an export-heavy user grows it without bound | parked. Reopen **only with a named size number** from a real machine, per the reopen trigger in [compare/view-export-and-run-stamp.compare.md](compare/view-export-and-run-stamp.compare.md) — never re-argued from first principles |
