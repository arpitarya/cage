...
5. **Rebuild the configuration, never the corpus — and the corpus is FROZEN.**
   ...
   **new named corpus alongside** it, and every result is labelled by which corpus
   ...
   is a separate open question: [proposal](../proposals/larger-lab-corpus.md).
...
| `cage/docs/regression/**` | **NEVER** — published, hashed, append-only. The lab's *results* live in cage, not in the lab |
