...
> Living spec: [FORMULAS.md §1.1a](../../docs/FORMULAS.md) · [PLAN.md §3.1](../../docs/PLAN.md) ·
> `cage query copilot-credits`. Every Definition-of-Done box below is met, with three
> recorded deviations — the §10 fallback taken (`[billing.copilot]`), the `credits`
> default changed to a `None` sentinel, and copilot-CLI stamping `credits` directly
> instead of the read side reusing `premium` (§5's CLI line was built on a field that
> floors its own value to zero —
> [finding](../research/2026-08-02-copilot-credit-fields-real-stores.md)). The archived
> [proposal](v0.44-copilot-credits.proposal.md) header explains all three.
...
Design of record: [proposals/copilot-credits.proposal.md](v0.44-copilot-credits.proposal.md)
...
- [ ] **compare doc** — no change (already DECIDED); **proposal** — archive per the
      lifecycle on green (`work/archive/vX.Y-copilot-credits.proposal.md`, header
