...
implementation and pinned by tests, and documented in [FORMULAS.md §1.1a](../../docs/FORMULAS.md),
[PLAN.md §3.1](../../docs/PLAN.md), [GLOSSARY.md](../../docs/GLOSSARY.md) and `cage query copilot-credits`.
...
`work/dogfood/` is built as that section's twin — same append-only/dated-snapshot
...
- **No entry in the maintained-doc enumeration.** `work/regression/` is not listed there
  either — it is covered by the `## Dev` section, and `work/dogfood/` follows that
...
*Raised 2026-08-03 by CHATS-AUTHOR (`work/archive/v0.46-chats-author.prompt.md` §9.5).*
