...
> Living spec: FORMULAS §2.13 (column contract) + §2.14 (`residual_lines`), PLAN §3.5,
> `cage query chats-view` / `cage query agent-authorship`.
>
> **Where the build corrected this proposal:** (a) its "demote behind `--authorship`
> if the golden overflows 100 cols" fork was **moot** — the chats table was already
> 113 cols before the column, so width could not be what decided it; default-on was
> confirmed. (b) A fourth refusal shape exists in fact and is folded into *no landed
> evidence*: a row that joins but carries no matchable line (a commit of only binary
> files) has no share to compute. (c) CSV `agent_pct` resolved to **0–100 with 1dp**,
> and **all three** authorship cells go empty on a refusal — not just the percentage.
...
> [chats-author.handoff.md](v0.46-chats-author.handoff.md) ·
> [chats-author.prompt.md](v0.46-chats-author.prompt.md) (Opus). The pair encodes the
> REV-TS sequencing as a hard Phase-0 gate: **the build STOPs unless
> [timestamp-utc-normal-form](v0.45-rev-ts.proposal.md) has landed.** This
...
- **Landed commits only.** A live chat's proposals show `—` until they are
  ...
  you can `git show`).
- **Inherits [timestamp-utc-normal-form](v0.45-rev-ts.proposal.md).** The
...
-
[FORMULAS §2.13 / §2.14 / §3](../../docs/FORMULAS.md).
