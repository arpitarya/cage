...
> `cage query copilot-credits`. Deviations are recorded in the archived
> [proposal](v0.44-copilot-credits.proposal.md) header.
...
**Progress:** 100% — all six task areas built and green (2026-08-02): substrate field ·
vscode + CLI capture · the one-choke-point ladder · the `[billing]` policy key ·
rendering (split footnote, second fix line, rate-unset advisory, chats column, CSV
`priced_via`, doctor line) · explain + FORMULAS + docs.
...
[docs/proposals/copilot-credits.proposal.md](v0.44-copilot-credits.proposal.md)
(its §4 example outputs are the rendering contract-by-example); the decision behind
it is [work/compare/copilot-pricing-basis.compare.md](compare/copilot-pricing-basis.compare.md)
