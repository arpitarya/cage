...
> The living spec is FORMULAS §2.13 (the `agent%` column contract) + §2.14 (the
> `residual_lines` count), PLAN §3.5, and `cage query chats-view` /
> `cage query agent-authorship`. Where this disagrees with them, they win.
...
([proposal](v0.45-rev-ts.proposal.md)) must land first and Phase 0
...
Design of record:
[proposals/chats-agent-authorship-column.proposal.md](v0.46-chats-author.proposal.md)
...
**Hard:** REV-TS ([timestamp-utc-normal-form](v0.45-rev-ts.proposal.md))
