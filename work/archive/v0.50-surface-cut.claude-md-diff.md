---
doc: the CLAUDE.md diff for SURFACE-CUT — APPLIED 2026-08-14
status: ARCHIVED — applied in full, then archived. History, never current spec.
pair: [surface-cut.decision.md](v0.50-surface-cut.decision.md)
---

> **APPLIED 2026-08-14 and archived.** Every correction below is now in `CLAUDE.md`,
> including section D's new rule (*a reader may be deleted; the writer it read is a
> separate decision*) and section C's rewrite of the Decision-records section against
> the seven-record ADR set. Kept as the record of **what was wrong and why**, which
> the applied diff no longer shows. History, never current spec.

# Proposed `CLAUDE.md` diff — SURFACE-CUT

**Applied 2026-08-14.** (Original text: *Not applied.*) CLAUDE.md is the always-loaded contract; the standing rule is that its
...
- **`docs/adr/0011-cage-measures-usage-not-cost.md`** (and every other numeric ADR path)
  now lives under [work/archive/adr/](adr/README.md). CLAUDE.md cites the old
