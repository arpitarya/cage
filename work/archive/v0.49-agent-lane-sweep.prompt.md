...
pair: [v0.49-agent-lane-sweep.handoff.md](v0.49-agent-lane-sweep.handoff.md)
---

> **ARCHIVED — implemented for v0.49 (unreleased) on 2026-08-11.** Progress reached
> 100%. History, never current spec; see the handoff's archive header for where the
> living spec now lives.
...
**Progress:** 100% — all seven phases resolved (2026-08-11). 7 phases total
...
5. **Docs, in the same change** — this repo treats a stale doc as a bug:
   ...
   - bump the matching rows in `work/DOC-REGISTRY.md`;
...
- If a phase turns out to need a **decision** rather than a fix, stop and ask me. Several
  ...
  standing rule is that a fork gets a `work/compare/` doc *before* a plan — never a call
  made silently inside a fix commit.

## When the last phase lands

Archive the pair to `work/archive/vX.Y-agent-lane-sweep.{handoff,prompt}.md` with the
one-line archive header, link it from that version's `CHANGELOG.md` entry ("Built
from: …"), update `docs/README.md` and `work/archive/README.md`, and set this prompt's
