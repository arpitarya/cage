> **ARCHIVED 2026-08-12 — the open-work queue was closed wholesale on Arpit's explicit
> instruction. READ THIS BEFORE ASSUMING THESE RULES ARE DEAD.**
>
> This file was **never open work** — it held the standing constraints that bind future
> work regardless of which item you pick up. It is archived here because the directory it
> lived in was emptied, **not** because the rules were lifted. Nothing below was migrated
> into `CLAUDE.md` in that change.
>
> **Still enforced mechanically, independent of this file** — these do not depend on the
> prose surviving: the no-number-moves layer gate (`tests/test_floor.py`), three-agents-
> always (`agents.SURFACES`, `tests/test_agents.py`, `tests/test_mcp_layer.py`), the
> live-parser detector (`cage/wiringscan.py`, `tests/test_cli_tiering.py`,
> `tests/test_cli_reference.py`), `outcome="auto"` never `ok` (`cage/attest.py`,
> `tests/test_hooks_layer.py`), and `attest.LIMIT`'s CLI-only claim (`cage/attest.py`).
> Most are additionally restated in `CLAUDE.md`'s Must-Know Rules and architecture
> bullets, which remain the living contract.
>
> **Carried by prose alone, and therefore genuinely at risk now:** the frozen lab corpus
> (`tinyshop` is never mutated; a new question gets a new named corpus), the "unverified
> on a real Copilot" honesty requirement on the lifecycle-gaps text, the
> `cage setup --hooks`-is-off-by-default trap, F2's UNTESTED copilot-VS-Code receipt
> limit, and the correction that `tests/test_portable_wiring.py` has never existed.
>
> **History, never spec.**
...
([ADR 0001](../../docs/adr/0001-ledger-team-aggregation-notes-not-external-sink.md)). Every
...
[L1-FIELD](v0.49-l1-field.item.md) resolves it, do not present L1's agent identity or auto task-close
...
[L1-FIELD](v0.49-l1-field.item.md) verifies. *(Rescued 2026-08-11 from the archived review-hardening
...
separate filed question: [proposal](v0.49-larger-lab-corpus.proposal.md).
