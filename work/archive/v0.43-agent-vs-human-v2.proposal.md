...
status: IMPLEMENTED — shipped in v0.43.0 — see ADR 0008
raised: 2026-08-01 (Arpit)
amended: 2026-08-02 (Arpit — commit views, line-match capture, §4 estimator lifted with guards)
supersedes-context: the Tier-1 human axis, removed v0.36 (v0.36-human-removal.handoff.md)
handoff: v0.43-agent-vs-human-v2.handoff.md · prompt: v0.43-agent-vs-human-v2.prompt.md
living-spec: adr/0008-line-match-authorship-counts-persisted-content-transient.md
---
> **ARCHIVED — IMPLEMENTED — shipped in v0.43.0, 2026-08-02.** The rationale behind
> the build, kept as evidence; **never** the current spec. That is
> [ADR 0008](adr/0008-line-match-authorship-counts-persisted-content-transient.md),
> [PLAN.md §3.5](../../docs/PLAN.md), [FORMULAS.md §2.14](../../docs/FORMULAS.md) and [CLI.md](../../docs/CLI.md).
> **Where this disagrees with them, they win.** It disagrees in one place worth naming:
> the two surfaces are spec'd here with a three-way `agent / human / unknown` split, and
> the build made it **four** — `unattributed` had to be separated from `human~`, because
> a single human bucket printed 76.6% on cage's own repo, 89% of it one commit of
> generated JSON ([dogfood](../regression/2026-08-02-p1-authorship-dogfood.md) §4). The
