...
status: ARCHIVED — IMPLEMENTED for v0.44 (unreleased), 2026-08-02
raised: 2026-08-02
verdict-from: ../compare/copilot-pricing-basis.compare.md
evidence: ../research/copilot-vscode-token-sources.md · ../research/2026-08-02-copilot-credit-fields-real-stores.md
---

> **ARCHIVED — implemented for v0.44 (unreleased) on 2026-08-02. History, not spec.**
> The living spec is **[FORMULAS.md §1.1a](../../docs/FORMULAS.md)** (the rung table + method
> tags) and **[PLAN.md §3.1](../../docs/PLAN.md)** (the `credits` substrate field), with
> `cage query copilot-credits` as the shipped explanation. Where this document
> disagrees with those, they win.
>
> **Three corrections the build made to this proposal** — each is the valuable part,
> so they are recorded here rather than silently diverging:
>
> 1. **The policy key is `[billing.copilot] usd_per_credit`, not `[credits.copilot]`**
>    (§2 below). `[credits]` is a *price section*: `policy.load` reads it from
>    `prices.toml` alone, so a rate written into `cage.toml` — where this proposal
>    correctly wanted it — would have merged as absent in every project with a prices
>    file, silently unpricing the ledger. The handoff §10 named this risk and
>    pre-authorized the fallback; the fallback was taken.
> 2. **`credits` defaults to `None`, not `0.0`** (§1). The handoff specified
>    `credits: float = 0.0`, which collides with this proposal's own requirement that a
>    recorded `0.0` be "a REAL zero … distinct from absent": the omit-at-default idiom
>    would have collapsed the two. A sentinel default keeps them apart.
> 3. **Copilot CLI stamps `credits` directly; the read side does NOT reuse `premium`**
>    (§1's CLI row). `totalPremiumRequests` is fractional (`0.33`) and `premium` is an
>    int that floored every real value to zero and dropped the key — 13 CLI rows in a
>    real ledger, not one carrying it. `premium` is left untouched and the ladder reads
>    `credits` on both surfaces.
>    [Finding](../research/2026-08-02-copilot-credit-fields-real-stores.md).
>
> Still deferred, with their triggers intact (§6): monthly included-allowance
> modeling · nano-AIU → USD · the `agentHostUsage` sidecar.
