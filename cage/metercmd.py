"""`cage data meter -- <cmd>` — run a command with the metering proxy in front (plan §7).
...
def run(root: Path, argv: list[str], upstream: str = _DEFAULT_UPSTREAM) -> int:
    argv = list(argv)
    if argv and argv[0] == "--":          # tolerate `cage data meter -- <cmd> …`
        argv = argv[1:]
    if not argv:
        print("cage data meter: nothing to run (usage: cage data meter -- <cmd> [args…])")
        return 2
    port = _free_port()
    ...
    env = {**os.environ, "ANTHROPIC_BASE_URL": base, "OPENAI_BASE_URL": base,
           "CAGE_ACTIVE": "1"}
    print(f"cage data meter: running under proxy {base} → {upstream}")
    try:
        return subprocess.run(argv, env=env).returncode
    finally:
        httpd.shutdown()
        httpd.server_close()
        print(f"cage data meter: done — `cage report` to see what {argv[0]!r} spent.")
