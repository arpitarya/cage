"""`cage insights why <call-id>` — full provenance: the call + every receipt against it (ADR-CLI)."""
from __future__ import annotations

from pathlib import Path

from cage import ledger, paths, policy, render


def explain(root: Path, call_id: str, pol: dict | None = None) -> dict:
    """`pol` is accepted and unused — kept so every caller's signature is unchanged now
    that no figure here is priced (USAGE-ONLY, ADR 0011)."""
