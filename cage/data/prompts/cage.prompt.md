...
- **spend** ("what did this cost") — `cage report` (add `--by model`, `--by day`,
  or `--since 7d`). Tokens are the default view; add `--usd` for dollar columns
  (or set `[display] usd = true` in policy for always-on). An unpriced cell
  renders `—`, never `$0.0000`; saved/net columns appear only when savings
  receipts exist in the window (`--all-columns` forces the full grid; CSV always
  carries every column).
- **savings** ("what saved money / which tool helped") — `cage insights attrib` (per-tool
  marginal) · `cage insights roi` (saved $ vs each tool's own cost).
- **the one-line answer** ("is tool X worth keeping") — `cage insights verdict <tool>`
  (SAVING / COSTING / INSUFFICIENT DATA, composed from existing views, every
  input method-tagged).
- **counterfactual** ("what would X have cost") — `cage insights matrix` (token grid by
  default — it always renders; `--usd` adds the cost column or one line
  explaining why it can't; `--human` implies `--usd`).
- **measured stack comparison** ("did tasks with the tool actually cost less") —
  `cage insights compare` (closed tasks grouped by observed stack; n · median · IQR, delta
  tagged `estimated` — observational, refuses tiny groups).
- **budget** ("am I over budget") — `cage insights budget`.
- **human attention** ("how much of my time did the agent eat") — `cage human show`
  (attested minutes vs `derived (turn-gaps, capped)` minutes on separate lines,
  never summed; both `estimated`). Attest the real figure with `cage human outcome
  <task> --minutes N`; `cage insights calibration --human` measures the heuristic's
  accuracy. `compare`/`verdict`/`study report` print a total-cost line (agent $
  + human minutes × rate) — suppress with `--agent-only`.
- **pre-task estimate** ("what will this cost") — `cage insights estimate [--label W]` (a
  `modeled` median+IQR band from matching closed tasks; refuses thin history;
  `--record <task>` stamps it so `cage insights calibration` can measure the hit-rate later).
- **explain a call** — `cage insights why <call-id>`.
...
- **fleet study** ("does the plugin pay off across our laptops") — `cage study
  join <phase>` per machine → `cage data export --study` → analyst runs `cage import
...
- per-tool savings — `cage insights attrib --csv` · ROI — `cage insights roi --csv`
- is tool X worth it — `cage insights verdict <tool>` and quote its verdict line verbatim
- fleet number — `cage study report --csv`
- estimate accuracy — `cage insights calibration --csv`
...
`cage data export --study` (jsonl) — never blur the two.

To meter this agent, run it under `cage data proxy` (point its base URL at the proxy),
or `cage data meter -- <cmd>` for a one-shot.
