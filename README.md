...
- **v0.31.0 (unreleased) — capture-on-read: capture without hooks, made visible.** Capture no longer depends on a hook firing. **Every read — `cage report`, `cage insights *`, the MCP read tools — lazily sweeps your agent logs into the ledger before it answers**, so a number is never staler than the instant it's shown (no daemon, no scheduler, no project required; throttled ~60s so back-to-back reads don't re-sweep; fail-open). Push (graphify/fux/proxy) and pull now resolve **one canonical ledger**, and a pushed saving carries a **non-PII routing key** (a hash of the ledger path, never a basename) so a repo-root read reclaims a graphify saving pushed from a subdirectory — by **exact key only**. Capture is now **visible**: a graphify saving prints `✔ cage: graphify saving captured — ~N tokens` to stderr, a read prints `· captured N new … since last read` (stderr; silent when nothing's new), the MCP tools return it as a structured field, and `cage doctor` shows a per-source, per-mode capture timeline. Suppress with `--no-import` / `CAGE_CAPTURE_ON_READ=0`; silence with `--quiet` / `CAGE_QUIET=1`; trace with `--why-ledger` / `CAGE_DEBUG=1`. Derived numbers stay a pure function of the ledger (byte-identical warm; CSV never gates). **Phase 1** — additive, no hook file touched; Phase 2 (deleting the token hooks) is a later release. See [Debugging capture](docs/debugging-capture.md).
